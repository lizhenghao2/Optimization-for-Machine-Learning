...
for i in range(max_iters):
    ...
    # Update x
    x -= step_sizes[0]/ np.sqrt(i + 1) * grad
...
# Reset x
x = np.zeros(A_normalized.shape[1])
print("\nStarting gradient descent with alpha =", step_sizes[1])
for i in range(max_iters):
    # Compute gradient using normalized data
    grad = - 1 / A_normalized.shape[0] * A_normalized.T @ (y_normalized - A_normalized @ x)
    # print(f"Iteration {i+1}: gradient norm = {np.linalg.norm(grad):.6e}")
    ...
    # Update x
    x -= step_sizes[1]/ np.sqrt(i + 1) * grad
...
# Reset x
x = np.zeros(A_normalized.shape[1])
print("\nStarting gradient descent with alpha =", step_sizes[2])
for i in range(max_iters):
    # Compute gradient using normalized data
    grad = - 1 / A_normalized.shape[0] * A_normalized.T @ (y_normalized - A_normalized @ x)
    # print(f"Iteration {i+1}: gradient norm = {np.linalg.norm(grad):.6e}")
    ...
    # Update x
    x -= step_sizes[2]/ np.sqrt(i + 1) * grad
...
plt.tight_layout()
plt.savefig('level_sets_2D_4c.png', dpi=150)

print(f"\nLevel set plot saved as 'level_sets_2D_4c.png'")
